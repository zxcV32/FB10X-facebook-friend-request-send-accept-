...
public class Configuration extends JFrame {
	...
	public Configuration(String osName,String botName) {
		...
		JLabel lblTimerangeGiven = new JLabel("Time range for two consicutive friend request sent/confirmed");
		...
		btnExecute.addActionListener(new ActionListener() {
			...
			try{	
				 ...
				 if(confirmations<1)
					...
					if(username.length()>0 && password.length()>0 && rangeMin>=1 && rangeMax<Integer.MAX_VALUE && confirmations<=500 && waveMin>0 && waveMax<Integer.MAX_VALUE && fpath!=null )
		...
		btnExecute.setBounds(150, 419, 97, 25);
		contentPane.add(btnExecute);
		
	}		Execute uInput = null;
	public void process(String osName,String botName){
		//JOptionPane.showMessageDialog(null,username.length()+" | "+password+" | "+ rangeMin+" | "+" | "+rangeMax+" | "+confirmations+" | "+waveMin+" | "+waveMax+" | "+fpath);
		dispose();
		try {
		...
		uInput = new Execute(osName,botName,username,password,rangeMin,rangeMax,confirmations,waveMin,waveMax,fpath);
